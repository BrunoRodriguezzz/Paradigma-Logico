% % No anda Bien.
% necesidad(respiracion, fisiologica).
% necesidad(descanso, fisiologica).
% necesidad(empleo, seguridad).
% necesidad(integridadFisica, seguridad).
% necesidad(salud,seguridad).
% necesidad(amistad, social).
% necesidad(afecto, social).                          % Jerarquia
% necesidad(intimidad, social).                       % Autorrealizacion
% necesidad(confianza,reconocimiento).                % Reconocimiento
% necesidad(respeto,reconocimiento).                  % Social
% necesidad(exito,reconocimiento).                    % Seguridad
% necesidad(autoestima, autorrealizacion).            % Fisiologico
...
% nivelEntreNecesidades(Necesidad1, Necesidad2, Nivel):-
%     necesidad(Necesidad1,Nivel1),
%     necesidad(Necesidad2,Nivel2),
%     diferenciaEntreNiveles(Nivel1,Nivel2,Nivel).
...
%     queNecesita(Persona,Necesidad),
%     necesidad(Necesidad, NivelNecesidad),
%     jerarquiaNivel(NivelNecesidad,Jerarquia),
%     forall(queNecesita(Persona,N), (necesidad(N, NivN), jerarquiaNivel(NivN, J), N \= Necesidad, Jerarquia > J)).

% %Mido todos desde la base
% jerarquiaNivel(Nivel,Jerarquia):-
%     diferenciaEntreNiveles(Nivel,fisiologica,Jerarquia).

% % necesidadMayorJerarquia(Persona,Necesidad):-          Anda por algun motivo.
% %     queNecesita(Persona,Necesidad),
% %     forall(queNecesita(Persona,N),(nivelEntreNecesidades(Necesidad,N,_))).

%6)

% motivacionPara1(Persona):-
%     forall(queNecesidad(Persona, Necesidad),noTieneNecesidadesPrevias(Necesidad, Persona)).

% noTieneNecesidadesPrevias(Necesidad, Persona):-
%     necesidad(Necesidad,Nivel),
%     jerarquiaNivel(Nivel,Jerarquia),
%     Jerarquia\=0,
%     nivelSuperior(Nivel,NivelInferior),
%     pudoSatisfacerAlgunNivel(Persona, NivelInferior).
